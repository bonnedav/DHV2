...
```""")
    ...
    async def duckhunt(self, ctx):
        ...
        if str(ctx.invoked_subcommand) == "faq duckhunt":
            ...
            embed.description = "DuckHunt is a game Eyesofcreeper developed. Named as the game on NES, the goal is to be the best duck hunter on a server."
    ...
    async def dh_noselfhost(self, ctx, user: discord.Member = None):
        ...
        embed.add_field(name="4th reason", value="If you self hosted and want to go back to the official version, you can't. I can't merge two databases, as this implies a security risk for me and the users.", inline=False)
        embed.add_field(name="Need more help ?", value="A lot of others reasons exist. You may discuss that with us on the official duckhunt discord server at https://discordapp.com/invite/2BksEkV", inline=False)
        # embed.set_image(url="https://api-d.com/duckhunt.jpg")
        embed.set_footer(text=self.footer, icon_url=self.bot.user.avatar_url)
        await send_embed(self.bot, ctx.message, embed, user)
